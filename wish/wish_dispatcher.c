...
void wish_core_send_pong(wish_context_t* ctx) {
    ...
    bson_write_boolean(pong_msg, pong_msg_max_len, 
        "pong", true);
    wish_core_send_message(ctx, pong_msg, bson_get_doc_len(pong_msg));
...
size_t wish_core_get_local_hostid(uint8_t *hostid_ptr) {
    ...
    if (config_fd < 0) {
        ...
        if (read_len < 0) {
            ...
            wish_core_create_hostid((char*)hostid_ptr, sys_id_input_str, WISH_WHID_LEN);
...
void wish_core_create_handshake_msg(uint8_t *buffer, size_t buffer_len) {
    ...
    wish_core_get_local_hostid(host_id);
...
void wish_core_process_handshake(wish_context_t* ctx, uint8_t* bson_doc) {
    ...
    wish_core_send_message(ctx, handshake_msg, bson_get_doc_len(handshake_msg));
...
void wish_core_process_message(wish_context_t* ctx, uint8_t* bson_doc) {
    ...
    if (bson_get_document(bson_doc, "req", &core_lvl_pl,
        ...
        wish_core_feed_to_rpc_server(ctx, core_lvl_pl, core_lvl_pl_len);
        ...
        wish_core_feed_to_rpc_client(ctx, core_lvl_pl, core_lvl_pl_len);
...
void wish_core_handle_app_to_core(uint8_t src_wsid[WISH_ID_LEN], uint8_t *data, size_t len) {
    ...
    if (bson_get_binary(data, "wsid", &recovered_wsid, &recovered_wsid_len)
        ...
        wish_service_register_add(src_wsid, name, protocols_array,
            permissions_array);
        ...
        bson_write_string(ready_signal, ready_signal_max_len, 
            "signal", "ready");
        send_core_to_app(src_wsid, ready_signal, ready_signal_max_len);
    }
        ...
        if (!app_ready) {
            ...
            struct wish_service_entry *service_entry = wish_service_get_entry(src_wsid);
            if (service_entry != NULL) {
                wish_send_peer_update(service_entry, true);
            }
            ...
            struct wish_service_entry *service_entry_ready = wish_service_get_entry(src_wsid);
            ...
            for (i = 0; i < WISH_MAX_SERVICES; i++) {
                if (wish_service_entry_is_valid(&(service_registry[i]))) {
                    ...
                    wish_send_peer_update_locals(src_wsid, &(service_registry[i]), true);
                    /* Inform the other services on this core about the new service which became ready */
                    wish_send_peer_update_locals(service_registry[i].wsid, service_entry_ready, true);
                }
            }
        }
    }
         ...
         * a remote service */
        wish_core_app_rpc_handle_req(src_wsid, data);
    }
}
