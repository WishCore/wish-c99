 ...
 */
void wish_ldiscover_feed(wish_ip_addr_t *ip, uint16_t port, uint8_t *buffer, 
size_t buffer_len) {
    ...
    WISHDEBUG(LOG_DEBUG, "LocalDiscovery checking cache.");
    uint32_t current_time = wish_time_get_relative();
    bool found = false;
    int free = -1;
    ...
    for(i=0; i<WISH_LOCAL_DISCOVERY_MAX; i++) {
        if (ldiscovery_db[i].occupied && current_time - ldiscovery_db[i].timestamp > 30) {
            ...
            WISHDEBUG(LOG_DEBUG, "LocalDiscovery dropped timed out entry.");
        }
        ...
        if (ldiscovery_db[i].occupied) {
            if( memcmp(&ldiscovery_db[i].ruid,  ruid, WISH_ID_LEN) == 0 && 
                ...
                WISHDEBUG(LOG_DEBUG, "Found entry. Updating timestamp");
                found = true;
    ...
    if (wish_load_identity(ruid, &discovered_id) > 0) {
        WISHDEBUG(LOG_DEBUG, "Auto-discovered uid is already in our contacts");
    }
    else {
        WISHDEBUG(LOG_DEBUG, "Ignoring auto discovery bcast for unknown uid");
        return;
    }
    ...
    wish_context_t *existing_conn_ctx = wish_core_lookup_ctx_by_luid_ruid_rhid(uid_list[0].uid, ruid, rhid);
    ...
    if (wish_core_get_local_hostid(local_hostid) == WISH_WHID_LEN) {
    ...
    wish_context_t *new_ctx = wish_core_start(my_uid, ruid);
    if (new_ctx != NULL) {
         ...
         * 'transports' */
        wish_open_connection(new_ctx, ip, tcp_port, false);
    }
 ...
 */
static int create_transports_array(uint8_t *transports_array, 
    ...
    return 0;
#else
    if (wish_get_host_ip_str(host_part, WISH_MAX_TRANSPORT_LEN)) {
        WISHDEBUG(LOG_CRITICAL, "Could not get Host IP addr");
        return 1;
    }
    wish_platform_sprintf(transport_url, "wish://%s:%d", host_part, wish_get_host_port());
...
void wish_ldiscover_advertize(uint8_t *my_uid) {
    ...
    if (create_transports_array(transports_array, transports_array_max_len)) {
    ...
    wish_send_advertizement(advert_with_magic, advert_with_magic_len);
}
