...
static void methods(wish_rpc_ctx* req, uint8_t* args) {
    ...
    struct wish_rpc_server_handler *h = core_app_rpc_server.list_head;
...
static void services_send(wish_rpc_ctx* req, uint8_t* args) {
    ...
    if (memcmp(rhid, local_hostid, WISH_WHID_LEN) == 0) {
        ...
        if (bs.err) {
            ...
            send_core_to_app(rsid, upcall_doc, bson_get_doc_len(upcall_doc));
        }
        return;
    }
    /* Destination is determined to be a remote service on a remote core. */
    wish_context_t *dst_ctx = wish_core_lookup_ctx_by_luid_ruid_rhid(luid, ruid, rhid);
    ...
    if (dst_ctx != NULL && dst_ctx->context_state == WISH_CONTEXT_CONNECTED) {
        ...
        int send_ret = wish_core_send_message(dst_ctx, req_buf, bson_get_doc_len(req_buf));
        if (send_ret != 0) {
...
static void identity_export_handler(wish_rpc_ctx* req, uint8_t* args) {
    ...
    if (bson_get_string(args, "1", &export_type_str, &export_type_str_len) != BSON_SUCCESS) {
        ...
        write_bson_error(req, 8, "Missing export type argument");
        return;
    }
...
static void connections_list_handler(wish_rpc_ctx* req, uint8_t* args) {
    ...
    wish_context_t *db = wish_core_get_connection_pool();
...
static void wld_friend_request_handler(wish_rpc_ctx* req, uint8_t* args) {
    ...
    wish_context_t *friend_req_ctx = wish_core_start(luid, ruid);
    ...
    wish_open_connection(friend_req_ctx, &(db[i].transport_ip), db[i].transport_port, false);
...
void wish_core_app_rpc_init(void) {
    wish_rpc_server_register(&core_app_rpc_server, &methods_handler);
    wish_rpc_server_register(&core_app_rpc_server, &version_handler);
    
    wish_rpc_server_register(&core_app_rpc_server, &services_send_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "services.list", services_list_handler);
    
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.list", identity_list_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.export", identity_export_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.import", identity_import_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.create", identity_create_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.get", identity_get_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "identity.remove", identity_remove_handler);
    wish_rpc_server_register(&core_app_rpc_server, &identity_sign_handler);
    wish_rpc_server_register(&core_app_rpc_server, &identity_verify_handler);
    
    wish_rpc_server_add_handler(&core_app_rpc_server, "connections.list", connections_list_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "connections.disconnect", connections_disconnect_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "connections.checkConnections", connections_check_connections);
    
    wish_rpc_server_add_handler(&core_app_rpc_server, "wld.list", wld_list_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "wld.clear", wld_clear_handler);
    wish_rpc_server_add_handler(&core_app_rpc_server, "wld.friendRequest", wld_friend_request_handler);
    
    wish_rpc_server_register(&core_app_rpc_server, &host_config_handler);
    
    //wish_rpc_server_add_handler(&core_app_rpc_server, "debug.enable", debug_enable);
    //wish_rpc_server_add_handler(&core_app_rpc_server, "debug.disable", debug_disable);
}

static void wish_core_app_rpc_send(void *ctx, uint8_t *data, int len) {
    uint8_t* wsid = ctx;
    send_core_to_app(wsid, data, len);
}
...
#endif
    struct wish_rpc_context_list_elem *list_elem = wish_rpc_server_get_free_rpc_ctx_elem(&core_app_rpc_server);
    if (list_elem == NULL) {
        ...
        rpc_ctx->server = &core_app_rpc_server;
        rpc_ctx->send = wish_core_app_rpc_send;
        rpc_ctx->send_context = src_wsid;
        memcpy(rpc_ctx->op_str, op_str, MAX_RPC_OP_LEN);
        rpc_ctx->id = id;
        rpc_ctx->local_wsid = src_wsid;
    
        if (wish_rpc_server_handle(&core_app_rpc_server, rpc_ctx, args)) {
...
void wish_core_app_rpc_cleanup_requests(uint8_t *wsid) {
    ...
    LL_FOREACH_SAFE(core_app_rpc_server.request_list_head, list_elem, tmp) {
...
#error not implemented
            //wish_platform_free....
#endif
            LL_DELETE(core_app_rpc_server.request_list_head, list_elem);
        }
    }
}
