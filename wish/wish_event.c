...
void wish_message_processor_task(struct wish_event *e) {
    switch (e->event_type) {
    ...
    case WISH_EVENT_CONTINUE:
    case WISH_EVENT_NEW_DATA:
        wish_core_process_data(wish_handle);
        break;
    case WISH_EVENT_NEW_CORE_CONNECTION:
        wish_core_send_peers_rpc_req(e->context);
        break;
    ...
    case WISH_EVENT_ACCEPT_FRIEND_REQUEST:
        if (e->context->curr_protocol_state 
            ...
            wish_core_handle_payload(e->context, NULL, 0);
        }
        else {
            WISHDEBUG(LOG_CRITICAL, "Unexpected state, closing connection!");
            wish_close_connection(e->context);
        }
