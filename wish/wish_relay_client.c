    ...
    case WISH_RELAY_CLIENT_WAIT:
        ...
        if (ring_buffer_length(&(rctx->rx_ringbuf)) >= 1) {
            ...
            case ':':
                ...
                wish_context_t *new_ctx = wish_core_start(null_id, null_id);
                 ...
                 * addresses */
                wish_open_connection(new_ctx, &(rctx->ip), rctx->port, true);
                break;
            default:
             ...
             * not in "timeout" */
            if (wish_time_get_relative() > (rctx->last_input_timestamp + RELAY_SERVER_TIMEOUT)) {
                WISHDEBUG(LOG_CRITICAL, "Relay control connection time-out");
                wish_relay_client_close(rctx);
            }
        }
...
void wish_relay_client_feed(wish_relay_client_ctx_t *rctx, 
    ...
    rctx->last_input_timestamp = wish_time_get_relative();
}
...
void wish_relay_check_timeout(void) {
    ...
    wish_relay_client_ctx_t *rctx = wish_relay_get_contexts();
    if (rctx->curr_state == WISH_RELAY_CLIENT_WAIT) {
        ...
        wish_relay_client_periodic(wish_relay_get_contexts());
    }
}
