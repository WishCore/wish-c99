...
void wish_service_register_add(uint8_t *src_wsid, char *name, 
    ...
    for (i = 0; i < WISH_MAX_SERVICES; i++) {
        if (wish_service_entry_is_valid(&(service_registry[i]))) {
            ...
            if (memcmp(src_wsid, service_registry[i].wsid, WISH_WSID_LEN) 
                    == 0) {
    ...
    for (i = 0; i < WISH_MAX_SERVICES; i++) {
        if (wish_service_entry_is_valid(&(service_registry[i]))
            ...
            memcpy(service_registry[i].wsid, src_wsid, WISH_WSID_LEN);
            strncpy(service_registry[i].service_name, name, 
            ...
            for (j = 0; j < WISH_APP_MAX_PROTOCOLS; j++) {
                ...
                if (bson_get_string(protocols_array, elem_name, 
                    ...
                    strncpy((char *) &(service_registry[i].protocols[0][j]),
                        protocol_name, WISH_PROTOCOL_NAME_MAX_LEN);
                }
                else {
                    WISHDEBUG(LOG_DEBUG, "No more protocols found for service %s", service_registry[i].service_name);
                }
            }
...
void wish_service_register_remove(uint8_t *wsid) {
    ...
    struct wish_service_entry *service_entry_offline = wish_service_get_entry(wsid);
    ...
    if (service_entry_offline != NULL) {
        ...
        wish_send_peer_update(service_entry_offline, false);
        /*  Send update to services on our local core */
        int i = 0;
        for (i = 0; i < WISH_MAX_SERVICES; i++) {
            if (wish_service_entry_is_valid(&(service_registry[i]))) {
                /* FIXME support for multiple protocols */
                if (strncmp(&(service_registry[i].protocols[0][0]), &(service_entry_offline->protocols[0][0]), WISH_PROTOCOL_NAME_MAX_LEN) != 0) {
                    /* Protocols do not match */
                    continue;
                }
                /* Inform the other services on this core about the service which became offline */
                wish_send_peer_update_locals(service_registry[i].wsid, service_entry_offline, false);
            }
        }
        ...
        wish_core_app_rpc_cleanup_requests(wsid);
    } else {
...
struct wish_service_entry *wish_service_get_entry(uint8_t *wsid) {
    ...
    for (i = 0; i < WISH_MAX_SERVICES; i++) {
        if (wish_service_entry_is_valid(&(service_registry[i]))) {
            /* We have found a valid entry in the service registery. Now
             * check if the wsids match */
            if (memcmp(wsid, service_registry[i].wsid, WISH_WSID_LEN) == 0) {
                retval = &(service_registry[i]);
            }
        }
    }
    return retval;
