...
void wish_connections_check(void) {
    ...
    for (j = 0; j < num_uids; j++) {
        if (i == j) { continue; }
        if( wish_core_is_connected_luid_ruid(uid_list[0].uid, uid_list[j].uid) ) { continue; }
        ...
        do {
            ...
            if (ret) {
                ...
                if (ret) {
                    ...
                    wish_connections_connect_tcp(uid_list[0].uid, uid_list[j].uid, &ip, port);
                }
            }
...
void wish_connections_connect_tcp(uint8_t *luid, uint8_t *ruid, wish_ip_addr_t *ip, uint16_t port) {
    ...
    if (new_ctx != NULL) {
         ...
         * 'transports' */
        wish_open_connection(new_ctx, ip, port, false);
    }
