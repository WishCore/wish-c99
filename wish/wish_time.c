...
static void check_connection_liveliness() {
    int i = 0;
    for (i = 0; i < WISH_CONTEXT_POOL_SZ; i++) {
        ...
        case WISH_CONTEXT_CONNECTED:
            ...
            if ((core_time > (w->latest_input_timestamp + PING_INTERVAL))
                ...
                bson_write_boolean(ping_msg, ping_msg_max_len, 
                    "ping", true);
                wish_core_send_message(w, ping_msg, bson_get_doc_len(ping_msg));
                w->ping_sent_timestamp = core_time;
            }

            if (core_time > (w->latest_input_timestamp + PING_TIMEOUT) &&
                (w->ping_sent_timestamp > w->latest_input_timestamp)) {
                WISHDEBUG(LOG_CRITICAL, "Connection ping: Killing connection because of \
inactivity");
                wish_close_connection(w);
            }
            break;
...
void wish_time_report_periodic(void) {
    ...
    check_connection_liveliness();
    wish_relay_check_timeout();

    static wish_time_t check_connections_timestamp;
    if (core_time > (check_connections_timestamp + 60)) {
        check_connections_timestamp = core_time;
