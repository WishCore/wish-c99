...
wish_context_t* wish_core_get_connection_pool() {
    return wish_context_pool;
}

/* Start an instance of wish communication */
wish_context_t* wish_core_start(uint8_t *local_wuid, uint8_t *remote_wuid) {
    ...
    for (i = 0; i < WISH_CONTEXT_POOL_SZ; i++) {
        ...
        if (w->context_state == WISH_CONTEXT_FREE) {
            ...
            /* Update timestamp */
            w->latest_input_timestamp = wish_time_get_relative();
            break;
        }
    }
...
wish_context_t* wish_core_lookup_ctx_by_connection_id(wish_connection_id_t id) {
 ...
 */
wish_context_t* 
wish_core_lookup_ctx_by_luid_ruid_rhid(uint8_t *luid, uint8_t *ruid,
        uint8_t *rhid) {
    wish_context_t *ctx = NULL;
    int i = 0;
...
bool wish_core_is_connected_luid_ruid(uint8_t *luid, uint8_t *ruid) {
    int i = 0;
...
void wish_core_feed(wish_context_t* h, unsigned char* data, int len) {
    ...
    if (rb_space >= len) {
        ...
        h->latest_input_timestamp = wish_time_get_relative();
   }
    else {
...
bool wish_core_check_wsid(wish_context_t* ctx, 
    uint8_t* dst_id, uint8_t* src_id) {
    bool retval = false;
 ...
 */
void wish_core_process_data(wish_context_t* h) {
again:
    ;
        ...
        case TRANSPORT_STATE_WAIT_PAYLOAD:
            ...
            if (ring_buffer_length(&(h->rx_ringbuf)) >= expect_payload_len) {
                ...
                if (buf != NULL) {
                    ...
                    wish_core_handle_payload(h, buf, h->expect_bytes);
...
connection type");
                    ...
                    if (wish_core_check_wsid(h, dst_id, src_id) == false) {
                    ...
                    if (h->server_dhm_ctx == 0) {
                        WISHDEBUG(LOG_CRITICAL, "Failed allocation near line \
%d", __LINE__);
                        wish_close_connection(h);
                        break;
                    }
...
void wish_core_register_send(wish_context_t* h, int (*send)(void *, unsigned char*, int), void* arg) {
    h->send = send;
    h->send_arg = arg;
}

void wish_core_signal_tcp_event(wish_context_t* h,  enum tcp_event ev) {
    ...
    case TCP_CONNECTED:
        ...
        if (h->friend_req_connection == false) {
            ...
            if (wish_load_identity_bson(h->local_wuid, my_identity, 
                    my_cert_max_len) < 0) {
...
offline signal because other connection luid and ruid found!");
                ...
                wish_send_online_offline_signal_to_apps(h, false);
            }
        }
        
        /* Delete any outstanding RPC request contexts */
        wish_cleanup_core_rpc_server(h);
...
void wish_core_handle_payload(wish_context_t* ctx, uint8_t* payload, int len) {
    ...
    case PROTO_STATE_ID_VERIFY_SEND_CLIENT_HASH:
         ...
         * a identity verification by sending the client hash */
        {
            ...
            wish_core_send_message(ctx, signature, ED25519_SIGNATURE_LEN);
            ...
            if (wish_core_decrypt(ctx, payload, ciphertxt_len, 
    ...
    case PROTO_STATE_WISH_HANDSHAKE:
        /* Remote ID has been verified, lets start setting up Wish */
        {
            ...
            if (wish_core_decrypt(ctx, payload, ciphertxt_len, auth_tag,
            ...
            wish_core_process_handshake(ctx, plaintxt);
    ...
    case PROTO_STATE_WISH_RUNNING:
        {
            ...
            int ret = wish_core_decrypt(ctx, payload, ciphertxt_len, 
            ...
            wish_core_process_message(ctx, plaintxt);
            wish_platform_free(plaintxt);
        }
        break;
    ...
    case PROTO_SERVER_STATE_WISH_SEND_HANDSHAKE:
        {
            ...
            wish_core_create_handshake_msg(handshake_msg, max_handshake_len);
           
            WISHDEBUG(LOG_DEBUG, "We have generated BSON message of len %d", 
                bson_get_doc_len(handshake_msg));

            if (wish_core_send_message(ctx, handshake_msg, 
                    bson_get_doc_len(handshake_msg))) {
...
wish send handshake");
    ...
    #error  Stop! Unimpelemnted feature! You must have \
    WISH_ALLOW_ALL_FRIEND_REQS or WISH_ACCEPT_ANY_FRIEND_REQ_IF_NO_FRIENDS

    #else
            WISHDEBUG(LOG_CRITICAL, "Since WISH_ALLOW_ALL_FRIEND_REQS is \
    defined, welcoming stranger with open arms!");
 ...
 */
int wish_core_send_message(wish_context_t* ctx, uint8_t* payload_clrtxt, int payload_len) {
...
int wish_core_decrypt(wish_context_t* ctx, uint8_t* ciphertxt, size_t 
...
wish_context_t* wish_identify_context(uint8_t rmt_ip[4], 
 ...
 */
void wish_core_init(void) {
    wish_core_init_rpc();
    wish_core_app_rpc_init();

}


int wish_core_get_rx_buffer_free(wish_context_t *ctx) {
...
void wish_close_all_connections(void) {
    ...
    for (i = 0; i < WISH_CONTEXT_POOL_SZ; i++) {
        ...
        case WISH_CONTEXT_IN_MAKING:
            /* FALLTHROUGH */
        case WISH_CONTEXT_CONNECTED:
            wish_close_connection(&(wish_context_pool[i]));
            break;
        case WISH_CONTEXT_CLOSING:
