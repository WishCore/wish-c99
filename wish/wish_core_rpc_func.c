...
wish_rpc_client_t core_rpc_client = { .next_id = 1 };
...
void wish_send_peer_update(struct wish_service_entry *service_entry, bool online) {
    int buffer_len = 300;
    uint8_t buffer[buffer_len];
    if (wish_service_entry_is_valid(service_entry)) {
        bson bs;
        ...
        wish_rpc_server_emit_broadcast(&core_rpc_server, "peers", bson_data(&bs), bson_size(&bs));
    }
}
        ...
        if (wish_service_entry_is_valid(&(registry[i]))) {
            ...
            if (strncmp(((const char*) &(registry[i].protocols[0][0])), 
                ...
                */
                ...
                send_core_to_app(registry[i].wsid, core_to_app, bson_get_doc_len(core_to_app));
            }
        }
    }
...
static void send_op_handler(struct wish_rpc_context *rpc_ctx, uint8_t *args_array) {
    ...
    send_core_to_app(dst_wsid, upcall_doc, bson_get_doc_len(upcall_doc));
}
...
void wish_core_init_rpc(void) {
    wish_rpc_server_add_handler(&core_rpc_server, "peers", peers_op_handler);
    wish_rpc_server_add_handler(&core_rpc_server, "send", send_op_handler);
}
...
void wish_core_connection_send(void* ctx, uint8_t *payload, int payload_len) {
    ...
    wish_core_send_message(wish_ctx, (uint8_t*)bson_data(&bs), bson_size(&bs));
}

/* Feed to core's RPC server. You should feed the document which is as the
 * element 'req' 
 */
void wish_core_feed_to_rpc_server(wish_context_t *ctx, uint8_t *data, size_t len) {
...
#endif
    struct wish_rpc_context_list_elem *list_elem = wish_rpc_server_get_free_rpc_ctx_elem(&core_rpc_server);
    if (list_elem == NULL) {
        ...
        rpc_ctx->server = &core_rpc_server;
        ...
        if (wish_rpc_server_handle(&core_rpc_server, rpc_ctx, args)) {
...
void wish_core_feed_to_rpc_client(wish_context_t *ctx, uint8_t *data, size_t len) {
    wish_rpc_client_handle_res(&core_rpc_client, ctx, data, len);
}
...
void wish_core_send_peers_rpc_req(wish_context_t *ctx) {
    ...
    wish_rpc_id_t id = wish_rpc_client(&core_rpc_client, "peers", NULL, 0, peers_callback,
        buffer, buffer_max_len);

    rpc_client_req* mreq = find_request_entry(&core_rpc_client, id);
    mreq->cb_context = ctx;
    ...
    wish_core_send_message(ctx, request, bson_get_doc_len(request));
}
        ...
        */
        ...
        for (i = 0; i < WISH_MAX_SERVICES; i++) {
            if (wish_service_entry_is_valid(&(registry[i]))) {
                ...
                send_core_to_app(registry[i].wsid,
...
void wish_cleanup_core_rpc_server(wish_context_t *ctx) {
    ...
    wish_rpc_client_end_by_ctx(&core_rpc_client, ctx);

    
    LL_FOREACH_SAFE(core_rpc_server.request_list_head, list_elem, tmp) {
        if (list_elem->request_ctx.ctx == (void*) ctx) {
            ...
            LL_DELETE(core_rpc_server.request_list_head, list_elem);
#ifdef WISH_RPC_SERVER_STATIC_REQUEST_POOL
