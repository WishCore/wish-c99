...
void connected_cb(wish_context_t *ctx) {
    ...
    wish_core_signal_tcp_event(ctx, TCP_CONNECTED);
}

void connected_cb_relay(wish_context_t *ctx) {
    //printf("Signaling relayed wish session connected \n");
    wish_core_signal_tcp_event(ctx, TCP_RELAY_SESSION_CONNECTED);
}
...
int wish_open_connection(wish_context_t *ctx, wish_ip_addr_t *ip, uint16_t port, bool relaying) {
    ...
    *(sockfd_ptr) = socket(AF_INET, SOCK_STREAM, 0);
    ...
    wish_core_register_send(ctx, write_to_socket, sockfd_ptr);
...
void setup_wish_server(void) {
    ...
    server_addr.sin_port = htons(wish_get_host_port());
...
int main(int argc, char** argv) {
    ...
    int num_ids = wish_get_num_uid_entries();
    //printf("Number of identities in db: %i\n", num_ids);

    /* Load local user database (UID list) */
    wish_uid_list_elem_t uid_list[num_ids];
    memset(uid_list, 0, sizeof (uid_list));
    int loaded_num_ids = wish_load_uid_list(uid_list, num_ids);
    ...
    for (i = 0; i < num_ids; i++) {
        ...
        if (as_client && remote_id_alias != NULL) {
            /* If we are client, and remote id name is set.. */
            if (strncmp(remote_id_alias, recovered_id.alias, 
                    WISH_MAX_ALIAS_LEN) == 0) {
                /* found the remote id we should use! */
                memcpy(&remote_identity, &recovered_id, 
                    sizeof (wish_identity_t));
            }
        }
    }
    ...
    if (as_client) {
        wish_context_t *new_ctx = wish_core_start(uid_list[0].uid, remote_identity.uid);
        uint8_t peer_ip[4] =  { 
        ...
        wish_open_connection(new_ctx, &ip, peer_port, false);
    }
    ...
    while (1) {
        ...
        if (select_ret > 0) {
            ...
            if (as_relay_client) {
                if (FD_ISSET(relay_sockfd, &wfds)) {
                    ...
                    if (connect_error == 0) {
                        ...
                        wish_relay_client_periodic(&relay_ctx);
                    }
                    else {
                ...
                if (FD_ISSET(relay_sockfd, &rfds)) {
                    ...
                    if (read_len > 0) {
                        wish_relay_client_feed(&relay_ctx, &byte, 1);
                        wish_relay_client_periodic(&relay_ctx);
                    }
                    else if (read_len == 0) {
                ...
                for (i = 0; i < NUM_APP_CONNECTIONS; i++) {
                    ...
                    if (FD_ISSET(app_fds[i], &rfds)) {
                        ...
                        if (read_len > 0) {
                            ...
                            app_connection_feed(i, buffer, read_len);
                        }
                        else if (read_len == 0) {
                            /* App has disconnected. Do clean-up */
                            //printf("App has disconnected\n");
                            app_connection_cleanup(i);
                            close(app_fds[i]);
                        }
                        else {
            ...
            for (i = 0; i < WISH_PORT_CONTEXT_POOL_SZ; i++) {
                ...
                if (FD_ISSET(sockfd, &rfds)) {
                    ...
                    int rb_free = wish_core_get_rx_buffer_free(ctx);
                    if (rb_free == 0) {
                    ...
                    if (read_len > 0) {
                        ...
                        wish_core_feed(ctx, buffer, read_len);
            ...
            if (as_server) {
                if (FD_ISSET(serverfd, &rfds)) {
                    ...
                    wish_context_t *ctx = wish_core_start(null_id, null_id);
                    if (ctx == NULL) {
                        ...
                        *fd_ptr = newsockfd;
                        ...
                        wish_core_register_send(ctx, write_to_socket, fd_ptr);
                        wish_core_signal_tcp_event(ctx, TCP_CLIENT_CONNECTED);
                    }
                }
            }
        ...
        if (advertize_own_uid && loaded_num_ids > 0) {
            ...
            if (time(NULL) > relay_check_timestamp + 10) {
                ...
                if (relay_ctx.curr_state == WISH_RELAY_CLIENT_INITIAL) {
                    if (loaded_num_ids > 0) {
                        ...
                        wish_relay_client_open(&relay_ctx, uid_list[0].uid);
                    }
                }
