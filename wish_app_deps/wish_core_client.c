...
void send_app_to_core(uint8_t *wsid, uint8_t *data, size_t len) {
    ...
    DL_FOREACH(wish_core_clients, elt) {
        ...
        if ( memcmp(wsid, elt->app->wsid, WISH_WSID_LEN) == 0) {
            ...
            //cl = elt;
            //printf("wish_core_client: send app to core found %s next: %p\n", elt->app->name, elt->next);
            break;
        }
    }
    
    if (tcp_stream != NULL) {
        //printf("%s to core... len: %i sid: %02x %02x %02x %02x tcp: %p\n", cl->app->name, (int)len, wsid[0], wsid[1], wsid[2], wsid[3], tcp_stream);
...
again:
    ...
    switch(state) {
        ...
        case 1:
            ...
            //printf("core_client::parse have a message for app %s data_len %i\n", client->app->name, expect);
            //bson_visit(data, elem_visitor);
