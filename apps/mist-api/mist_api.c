...
static struct wish_rpc_context_list_elem request_pool[MIST_API_REQUEST_POOL_SIZE];

wish_rpc_client_t client = { .next_id = 1 };
wish_rpc_server_t server = { .server_name = "mist-api", 
                             .rpc_ctx_pool = request_pool, 
                             .rpc_ctx_pool_num_slots = MIST_API_REQUEST_POOL_SIZE };

// instance
mist_api_t mist_api_inst;
...
static void methods(wish_rpc_ctx* req, uint8_t* args) {
    ...
    struct wish_rpc_server_handler *h = server.list_head;
...
static void online(void* ctx, wish_protocol_peer_t* peer) {
    ...
    wish_rpc_server_emit_broadcast(&server, "signals", bson_data(&b), bson_size(&b));
    ...
    if (changed) {
        wish_rpc_server_emit_broadcast(&server, "sandboxed.signals", bson_data(&b), bson_size(&b));
    }
}
...
static void mist_passthrough(wish_rpc_ctx *req, uint8_t *args) {
    ...
    //WISHDEBUG(LOG_CRITICAL, "mist passthrough request re-written:");
    //bson_visit((char*)bson_data(&b), elem_visitor);
    
    //mist_app_request(mist_api->mist_app, peer, (char*)bson_data(&b), bson_size(&b), mist_passthrough_cb);
    rpc_client_callback cb = req->send_context;
...
static void mist_map(wish_rpc_ctx *req, uint8_t *args) {
    ...
    wish_api_request(&b, mist_map_cb);
...
static void mist_request_mapping(wish_rpc_ctx *req, uint8_t *args) {
    ...
    if (dst_export != NULL) {
        ...
        coercion_counter++;
        mist_api_request(&bs, mist_request_mapping_cb);
    } else {
...
static sandbox_t* mist_sandbox_by_id(char* id) {
    sandbox_t* elt;
...
static void sandbox_passthrough(wish_rpc_ctx *req, uint8_t *args) {
    ...
    sandbox_t* sandbox = mist_sandbox_by_id(sandbox_id);
...
static void sandbox_signals_emit(char* signal) { //, bson* elem) {
    int buf_len = 300;
    char buf[buf_len];
...
static void mist_sandbox_add_peer(wish_rpc_ctx *req, uint8_t *args) {
    ...
    if ( success ) {
        ...
        if (added) { sandbox_signals_emit("peers"); }
    } else {
...
static void mist_sandbox_remove_peer(wish_rpc_ctx *req, uint8_t *args) {
    ...
    if ( success ) {
        ...
        if (removed) { sandbox_signals_emit("peers"); }
    } else {
        //WISHDEBUG(LOG_CRITICAL, "  Peer invalid.");
        //bson_visit(args, elem_visitor);
        wish_rpc_server_error(req, 57, "Peer invalid.");
...
static void sandbox_login(wish_rpc_ctx *req, uint8_t *args) {
    //WISHDEBUG(LOG_CRITICAL, "mist.sandbox.register");
    if (mist_api->wish_app == NULL) {
        WISHDEBUG(LOG_CRITICAL, "Error: Cannot satify request because mist_api->wish_app is NULL");
        return;
    }
    ...
    sandbox_t* sandbox = mist_sandbox_by_id(id);
...
static void update_identity_export_db() {
    ...
    for (i=0; i<MIST_API_MAX_UIDS; i++) {
        ...
        if ( identities[i].occupied != 0 && identities[i].export_occupied == 0 ) {
            ...
            wish_api_request(&b, identity_export_cb);
            //return;
        }
    }
}
...
static void wish_app_ready_cb(wish_app_t* app, bool ready) {
    ...
    if (ready) {
        ...
        wish_api_request(&b, identity_list_cb);
    } else {
        int buf_len = 300;
        char buf[buf_len];
...
static void periodic_cb(void* ctx) {
    //WISHDEBUG(LOG_CRITICAL, "Periodic %p %p", ctx, mist_api->periodic);
    if (mist_api->periodic != NULL) {
        mist_api->periodic(mist_api->periodic_ctx);
    }
}

static void mist_api_init_rpc(void) {
    if (server.list_head != NULL) {
        return;
    }
    
    wish_rpc_server_register(&server, &methods_handler);
    
    wish_rpc_server_register(&server, &mist_list_services_handler);
    wish_rpc_server_register(&server, &mist_signals_handler);
    wish_rpc_server_register(&server, &mist_ready_handler);
    //wish_rpc_server_register(&server, &mist_load_app_handler);
    wish_rpc_server_register(&server, &mist_get_service_id_handler);
    
    wish_rpc_server_register(&server, &control_read_handler);
    wish_rpc_server_register(&server, &control_write_handler);
    wish_rpc_server_register(&server, &control_invoke_handler);
    wish_rpc_server_register(&server, &control_follow_handler);
    wish_rpc_server_register(&server, &control_model_handler);
    wish_rpc_server_register(&server, &control_map_handler);
    wish_rpc_server_register(&server, &control_un_map_handler);
    wish_rpc_server_register(&server, &control_notify_handler);
    wish_rpc_server_register(&server, &control_request_mapping_handler);
    
    wish_rpc_server_register(&server, &manage_claim_handler);
    wish_rpc_server_register(&server, &manage_peers_handler);
    wish_rpc_server_register(&server, &manage_acl_model_handler);
    wish_rpc_server_register(&server, &manage_acl_allow_handler);
    wish_rpc_server_register(&server, &manage_acl_remove_allow_handler);
    wish_rpc_server_register(&server, &manage_acl_add_user_roles_handler);
    wish_rpc_server_register(&server, &manage_acl_remove_user_roles_handler);
    wish_rpc_server_register(&server, &manage_acl_user_roles_handler);
    wish_rpc_server_register(&server, &manage_user_ensure_handler);

    wish_rpc_server_register(&server, &mist_sandbox_list_handler);
    wish_rpc_server_register(&server, &mist_sandbox_remove_handler);
    wish_rpc_server_register(&server, &mist_sandbox_list_peers_handler);
    wish_rpc_server_register(&server, &mist_sandbox_add_peer_handler);
    wish_rpc_server_register(&server, &mist_sandbox_remove_peer_handler);
    
    wish_rpc_server_register(&server, &sandbox_login_handler);
    wish_rpc_server_register(&server, &sandbox_logout_handler);
    wish_rpc_server_register(&server, &sandbox_settings_handler);
    wish_rpc_server_register(&server, &sandbox_list_peers_handler);
    wish_rpc_server_register(&server, &sandbox_signals_handler);
    wish_rpc_server_register(&server, &sandbox_control_model_handler);
    wish_rpc_server_register(&server, &sandbox_control_follow_handler);
    wish_rpc_server_register(&server, &sandbox_control_read_handler);
    wish_rpc_server_register(&server, &sandbox_control_write_handler);
    wish_rpc_server_register(&server, &sandbox_control_invoke_handler);
}
...
int wish_api_request(bson* req, rpc_client_callback cb) {
    return wish_rpc_passthru(&mist_api->wish_app->rpc_client, req, cb);
}
...
static void send_north(void* ctx, uint8_t *payload, int payload_len) {
    ...
    rpc_client_callback cb = ctx;
    //WISHDEBUG(LOG_CRITICAL, "Sending this northward:");
    //bson_visit(payload, elem_visitor);
    cb(NULL, NULL, payload, payload_len);
}
...
int mist_api_request(bson* req, rpc_client_callback cb) {
    ...
    if (wish_rpc_server_handle(&server, &rpc_ctx, args)) {
        WISHDEBUG(LOG_CRITICAL, "RPC server fail");
    }
#else
