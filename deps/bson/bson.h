...
enum bson_flags_t {
    ...
    BSON_FLAG_EXTERNAL_BUFFER = 1 << 2  /**< Force to use given buffer */
};
 ...
 * @return the type of the found object or BSON_EOO if it is not found.
 */
bson_type bson_find_fieldpath_value(const char *fieldpath, bson_iterator *it);
...
void bson_visit_fields(bson_iterator *it, 
...
/**
 * Get pointer to position in data
 *
 * @param i the iterator.
 *
 * @return returns current cursor poisition in bson data or NULL
 */
...
/**
 * Append at most max bytes of a string to a bson.
 *
 * @param b the bson to append to.
 * @param name the key for the string.
 * @param str the string to append.
 * @param max the number of bytes from str to append.
 *
 * @return BSON_OK or BSON_ERROR.
 */
int bson_append_string_maxlen(bson *b, const char *name, const char *str, int max);
...
int bson_compare_fpaths(const void *bsdata1, const void *bsdata2, 
...
bson* bson_create_from_buffer2(bson *bs, const void *buf, int bufsz);
...
int bson_merge_arrays(const void *mbuf, const void *inbuf, 
...
/**
 * Append all elements from iterator
 * 
 * @param bs
 * @param from
 */
